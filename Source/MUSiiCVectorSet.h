...
protected:
	...
	igtl::SmartPointer<igtl::MUSiiCVector<igtl::SmartPointer<igtl::MUSiiCVector<Data> > > >	m_MVector;
...
public:
	...
	MUSiiCVectorSet()
	{
		m_MVector		= igtl::MUSiiCVector<igtl::SmartPointer<igtl::MUSiiCVector<Data> > >::New();
		...
		m_bSelfMutex    = true;
		m_bIsVectors    = false;
		//m_bIsInputVectors = false;																
		//m_bIsOutputVectors = false;
	...
	~MUSiiCVectorSet()
	{
		/*this->ClearMUSiiCVectorSet();
		m_Cond.~SmartPointer();*/
		if(this->m_bSelfMutex)
		{
			if(m_bDebug) 
	...
	/// Add an object of MUSiiCVector with Input Data Interface
	/// return value is the ID of new MUSiiCVector
	int AddMUSiiCVectorInputDataIF(igtl::SmartPointer<igtl::MUSiiCVector<Data> >  pMVector)
	{
		 int s(0);
		 if(pMVector.IsNotNull())
		 {
	...
	/// Add an object of MUSiiCVector with Output Data Interface
	/// return value is the ID of new MUSiiCVector
	int AddMUSiiCVectorOutputDataIF(igtl::SmartPointer<igtl::MUSiiCVector<Data> > pMVector)
	{
		 int s(0);
		 if(pMVector.IsNotNull())
		 {
	...
	igtl::SmartPointer<igtl::MUSiiCVector<Data> > GetMUSiiCVector(int index= 0)
	{
		//if(m_bISVector && CheckIndex(index))
		if(CheckIndex(index))
		{
			igtl::SmartPointer<igtl::MUSiiCVector<Data> > tempVector = NULL;
	...
	bool RelaseMUSiiCVector(int index)
	{
		if(m_bIsVectors && CheckIndex(index))
		{
			igtl::SmartPointer<igtl::MUSiiCVector<Data> > tempVector = NULL;
			m_MVector->concurrent_try_get(tempVector, index);
			if(tempVector.IsNotNull())// && tempVector->IsWaiting())
			{
	...
	{
		//if(m_bIsVectors && CheckIndex(index)) cannon96 To do check :
		if(CheckIndex(index))
		{
			igtl::SmartPointer<igtl::MUSiiCVector<Data> > tempVector = NULL;
			m_MVector->concurrent_try_get(tempVector, index);
	...
	//// Remove a MUSiiCVector //TO_DO: Do Unit Test
	int RemoveMUSiiCVector(igtl::SmartPointer<igtl::MUSiiCVector<Data> > pMVector)
	{
	...
	void EnforceRelease()
	{
		...
		{
			if(m_bDebug)
				std::cout << "MUSiiCVectorSet EnforceRelease" << std::endl;
			m_bTerminate = true;
			m_Cond->Signal();
		}
	}
