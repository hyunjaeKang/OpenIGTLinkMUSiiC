...
//bool SetReferenceData(igtl::MessageBase::Pointer msg)
//{
//	if(msg.IsNotNull())
//	{
//		std::string device(msg->GetDeviceName());
//		std::string REF("M_REF_");
//		device = REF + device;
...
int main(int argc, char** argv)
{
	...
	//igtl::USMessage::Pointer us = GenerateDummyUSMessage();
	////// Receive US Data
	////igtl::MessageBase::Pointer msg0 = igtl::MessageBase::New();
	//igtl::MessageBase::Pointer msg0 = (igtl::MessageBase::Pointer)(us);
	////// Retrieve USMessage from MessgeBase 
	////igtl::ImageMessage::Pointer img = RecoverIGTLMSg2<igtl::ImageMessage::Pointer, igtl::ImageMessage>(msg0);///This part cause error
	//igtl::USMessage::Pointer img = RecoverIGTLMSg2<igtl::USMessage::Pointer, igtl::USMessage>(msg0);
	//igtl::Matrix4x4 matrix0;
	//img->GetMatrix(matrix0);
	//
	//std::cerr << "Before updating **************" << std::endl;
	//igtl::PrintMatrix(matrix0);
	//std::cerr << " " << std::endl;
	...
	//igtl::Matrix4x4 matrix1;
	//GetMatrix2(matrix1);
	//img->SetMatrix(matrix1); /// Update Matrix
	//std::cerr << "Mew Matrix **************" << std::endl;
	//igtl::PrintMatrix(matrix1);
	//std::cerr << " " << std::endl;

	////// Convert USMessage to MessageBase
	////igtl::MessageBase::Pointer msg1 = igtl::MessageBase::New();
	//igtl::MessageBase::Pointer msg1 = (igtl::MessageBase::Pointer)(img);
	///// Retrieve USMessage from MessageBase
	//igtl::USMessage::Pointer us3 = RecoverIGTLMSg2<igtl::USMessage::Pointer, igtl::USMessage>(msg1);
	//igtl::Matrix4x4 matrix2;
	//us3->GetMatrix(matrix2);
	//std::cerr << "After updating **************" << std::endl;
	//igtl::PrintMatrix(matrix2);  ///Error
	//std::cerr << " " << std::endl;
	...
	igtl::MessageBase::Pointer msg1 = (igtl::MessageBase::Pointer)(us);
	std::cerr << "The Device Type of MessageBase is " << msg1->GetDeviceType() <<std::endl;
	std::cerr << "The Device Name of MessageBase is " << msg1->GetDeviceName() <<std::endl;
	
	//std::string device(msg1->GetDeviceName());
	//std::string REF("M_REF_");

	////device += REF;
	//device = REF + device;
	//msg1->SetDeviceName(device.c_str());

	if(IsReferenceData(msg1))
		std::cerr << "This is Reference Data " << std::endl;
	else
		std::cerr << "This is not Reference Data " << std::endl;

	SetReferenceData(msg1);
	
	//// Retrieve USMessage from MessgeBase 
	//igtl::ImageMessage::Pointer img = RecoverIGTLMSg2<igtl::ImageMessage::Pointer, igtl::ImageMessage>(msg0);///This part cause error
	igtl::USMessage::Pointer img = RetrieveMessage<igtl::USMessage::Pointer, igtl::USMessage>(msg1);
	igtl::Matrix4x4 matrix0;
	img->GetMatrix(matrix0);
	...
	std::cerr << "The Device Type of MessageBase is " << msg2->GetDeviceType() <<std::endl;
	std::cerr << "The Device Name of MessageBase is " << msg2->GetDeviceName() <<std::endl;

	if(IsReferenceData(msg2))
		std::cerr << "This is Reference Data " << std::endl;
	else
		std::cerr << "This is not Reference Data " << std::endl;

	/// Retrieve USMessage from MessageBase
	igtl::USMessage::Pointer us3 = RetrieveMessage<igtl::USMessage::Pointer, igtl::USMessage>(msg2);
	igtl::Matrix4x4 matrix2;
	us3->GetMatrix(matrix2);
	std::cerr << "After updating **************" << std::endl;
	igtl::PrintMatrix(matrix2);  ///Error
