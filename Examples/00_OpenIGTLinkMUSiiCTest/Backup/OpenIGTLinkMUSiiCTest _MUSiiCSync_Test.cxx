...
public:
	...
	int GetSize(){ return testINT.size();}
...
int main(int argc, char** argv)
{
	...
	////////////////////////////////////////////
	/// Vector Test////////////////////////////
	std::vector<int> testINT2;

	{
		VectorTest vs;
		int r(vs.FillUp(5));

		testINT2 = vs.GetVector();
		int ss = vs.GetSize();	
	...
	////////////////////////////////////////////
	/// Simple Filter Test
	igtl::MessageBase::Pointer Dimg = GetDummyImageMessage();
	igtl::MessageBase::Pointer Dus = GetDummyUSMessage();
	igtl::MessageBase::Pointer Dei = GetDummyEIMessage();
	igtl::MessageBase::Pointer DTrack = GetDummyTrackingDataMessage();
	igtl::MessageBase::Pointer DStr = GetDummyStringMessage();

	bool result(false);

	result = IsImageMessage(Dimg);
	result = IsTrackingDataMessage(Dimg);

	result = IsImageMessage(Dus);
	result = IsTrackingDataMessage(Dus);

	result = IsImageMessage(Dei);
	result = IsTrackingDataMessage(Dei);

	result = IsImageMessage(DTrack);
	result = IsTrackingDataMessage(DTrack);

	result = IsImageMessage(DStr);
	result = IsTrackingDataMessage(DStr);
